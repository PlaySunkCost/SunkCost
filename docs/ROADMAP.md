...
| ~~The seven~~ | Done as placeholders: the Elevator Ghost (green car on a return trip, the doors slam), the Long Walker (sees you, walks forever, 85 % of walking, stops at the car), the Weeping Angel (frozen while watched, sprints unwatched), the Charger (1.5 s shake, a 12 m rush, 35 HP + a leak), the Lure (a bolt of light at a lit lamp; lamps off and it forgets), the Listener (blind; a dark bolt at what the noise bus says; crouch is silent), the Impostor (seen by its chosen diver only, wears a crewmate's colour and name, 30 HP + a leak, then runs). | — | `docs/test-runs/2026-09-20-monsters`. |
...
| The dash key | Dan: the intended dodge for the Charger; "the dash is later". A short burst on a key with a cooldown; costs air; a noise event for the Listener. | M | Design §6 names it. |
...
| Tuning from play | Every number is in `MonsterSettings` (Resources) and provisional: three per dive, the Angel's 25 m, the Charger's 35, the Walker's 85 %. | S | Playtest-driven. |
