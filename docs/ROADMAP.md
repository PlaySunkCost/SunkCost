...
| ~~The dash key~~ | Done 21 September 2026 (`dan/dash`): Alt, 6 m in 0.25 s in the steering direction with a little lift (Dan: longer, "like a dash in water"), 3 s apart, 4 s of tank, a 20 m noise; client-simulated, judged by the server from the copy's speed; rings out of the boots and a whoosh on every screen; DASH (ALT) on the visor. The air dash is in for Dan to try. | — | Design §3; contract §3 "The dash". |
...
| Monster looks and signature sounds | **Looks: five of seven in** (24 September 2026, `dan/monster-look`): the Listener, Lure, Weeping Angel, Long Walker and Charger wear Dan's Meshy generations, rigged with their clips; the Impostor waits for the diver's rig, the Ghost is the car. Still owed: the calls (a sound designer: the Ghost's hum, the slam) and a Play Mode pass (docs/test-runs/2026-09-24-monster-look). | L | Phase 03. |
...
| Card | Why | Size | Notes |
|---|---|---|---|
| Art pass 1: HQ props from Blender | 24 generated models prepared and assembled on `codex/hq-generated-art`, with existing ship props/materials and a connected equipment depot. See [HQ art handoff](HQ_GENERATED_ART_HANDOFF.md). | L | Local HQ, shop, plank and world-loop checks passed; Steam playtest, teammate review and integration pending. |
| Art pass 2: the ship | Hull, tower, the tube's housing, the storage room, the TV. | L | Dan. Done 23 September 2026 (`dan/ship-look`): thirty generated parts on a 48 x 20 m deck. The ship audit's fixes followed the same day with Dan's decisions (sizes, the well's rail, the lounge, the lowered TV, sitting on the couch; DESIGN.md §1 "The ship"), in edit mode only: the Play Mode runs are owed (`docs/test-runs/2026-09-23-ship-look/RESULT.md`). |
| The way aboard from the HQ | Implemented on `codex/hq-generated-art` (25 September 2026): HQ lowered to the current ship deck height, fixed level bridge to a gated port opening. No stairs or tower jump. See [HQ art handoff](HQ_GENERATED_ART_HANDOFF.md) for verification; integration and teammate review pending. | M | User-approved level bridge. |
| How high the ship floats | Done 23 September 2026 (Dan: "make the ship move above the water"): the sea is 4.5 m under the deck at sea, one number in `ShipStubBuilder.SeaLevelY` mirrored by `DiveSiteSettings.seaLevelY`, the car and `PlayerSubmersion`'s fallback. The ride's dry stretch grew from 1 m to 4.5 m; the cabin matrix is the check. The generated HQ branch uses the same `ShipStubBuilder.SeaLevelY` for HQ water so boarding is level and the ship floats at the same height in both worlds. | M | |
