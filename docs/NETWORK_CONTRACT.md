...
| Monster AI and targeting | **Server only** | Clients receive positions and animation state. **Built 20 September 2026 (docs/DESIGN.md §6 "The monsters"):** every walker is a `Creature` (`NetworkBehaviour`) on a registered prefab the server spawns into the dive scene with no owner (`MonsterRoster` on the day state: `monstersPerDive` distinct kinds drawn with `UnityEngine.Random` once the first diver has landed, never replicated as a roll — the spawns are the truth; they despawn with the site). The server runs the brain every frame (`ServerThink`), moves the body with a `CharacterController` on the **Monster** layer (solid to the world only: the layer ignores Player, Carryable and itself, so a touch is a flat distance, never physics), hears through `INoiseListener` on the bus, sees through `CreatureSenses` (a lamp-lit diver to `sightMeters`, a dark one to `sightDarkMeters`, a raycast on the world mask minus monsters; "watched" for the Angel is the diver's `EyePose` cone within `watchHalfAngleDeg` and a clear line). Replicated: the position and yaw by a **server-authoritative** `NetworkTransform` (`_clientAuthoritative` off — the first such prefab; a client copy's controller is disabled so the transform is written for it), `pose` (a byte, `CreaturePose`), `targetId` (the diver it is after), `strikeSerial` (a hit landed); the Lure and the Listener carry `CreatureBolts` with a one-shot `BeamCue { Serial, From, To, StartTick, Dark, Fired }` SyncVar and a `hitSerial` — the aim (`Fired` false: every peer draws the faint line, the tell) and `beamAimSeconds` later the shot (`Fired` true, the flash; `MonsterBeamView`, presentation); the server alone judges the hit at the moment of the shot: the first diver within `beamHitRadius` of the line before the first wall, not on safe ground, with a clear line from the beam to the chest. "Watched" for the Angel is any of its body's bounds' corners inside a living diver's camera frustum (`watchVerticalFovDeg`, the widest assumed `watchAspect`, `watchPadDeg`) with a clear line. The Impostor adds `wornColour` (a palette index) and `wornName`; **who sees it is decided per camera on every peer** (`ImpostorLook`, like `SpectatorView`'s head): the chosen diver's own camera, a dead spectator's watching that diver, the deck TV's while its channel is that diver — the object itself replicates to everyone (no observer condition; the repo's observer rebuilds have bitten before). Deaths go through `WorldSceneFlow.ServerKill` (the killers) and `ServerDamage` (the rest); a creature never comes within `safeZoneMeters` of the shaft and never strikes a diver in the car or on the tube's floor. The look (`CreatureLook`: eyes by pose, bob, shake) and the sounds (`CreatureSounds`) read the SyncVars and decide nothing. The editor's checks force the draw with `MonsterSettings.RosterOverrideForTests` (every other matrix runs with an empty draw) |
