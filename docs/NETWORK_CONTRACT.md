...
| Oxygen, health, damage | **Server only** | Clients display, never compute. **Damage and leaks (20 September 2026, the monsters):** `PlayerVitals.ServerDamage(points, leak, cause)` is the one way a creature hurts a diver — a living one with the suit on, health floored at 1 while riding, the ordinary `ServerKill` at 0; `leaking` is a server-written `SyncVar<bool>` (the tank drains at `leakDrainMultiplier`, 3×, on top of the sprint multiplier) cleared by `ServerBeginDive`, `ServerEndDive` and `ServerRevive`; `friendPatchDay` (`SyncVar<int>`, the run day a friend's hands last closed this diver's leak) makes the once-a-day rule readable on every peer for the prompt. **Patching:** the helper's `[ServerRpc] ServerRequestPatch(NetworkObject patient)` on its own `PlayerVitals` (the owner held E for `teammatePatchSeconds` first — the hold is the owner's, the server never sees it) — checked on the server: both alive with the suit on, the patient within `teammatePatchReach` + 1.5 m of the helper (copies a tick behind), a leak to close, not already patched by a friend today (`ServerPatchLeak(byTeammate: true)`); the answer goes back by `TargetRpc` to the helper and to the patient (a notice, nothing else). The patch kit is a `Patch` use action through the existing `ServerRequestUse`: `PatchKitItem.ServerPatch(holder)` — the suit on, a leak — closes the holder's own leak and flips its `used` SyncVar (name, use action and look derive from it through `IItemTag`; a used kit saves as `empty` like a used tank). Clients read `leaking` for the LEAK legend and the hiss (`PlayerLeakSounds`, presentation) |
| Loot value, quota, funds | **Server only** | Never trust a client number |
| Monster AI and targeting | **Server only** | Clients receive positions and animation state. **Built 20 September 2026 (docs/DESIGN.md §6 "The monsters"):** every walker is a `Creature` (`NetworkBehaviour`) on a registered prefab the server spawns into the dive scene with no owner (`MonsterRoster` on the day state: `monstersPerDive` distinct kinds drawn with `UnityEngine.Random` once the first diver has landed, never replicated as a roll — the spawns are the truth; they despawn with the site). The server runs the brain every frame (`ServerThink`), moves the body with a `CharacterController` on the **Monster** layer (solid to the world only: the layer ignores Player, Carryable and itself, so a touch is a flat distance, never physics), hears through `INoiseListener` on the bus, sees through `CreatureSenses` (a lamp-lit diver to `sightMeters`, a dark one to `sightDarkMeters`, a raycast on the world mask minus monsters; "watched" for the Angel is the diver's `EyePose` cone within `watchHalfAngleDeg` and a clear line). Replicated: the position and yaw by a **server-authoritative** `NetworkTransform` (`_clientAuthoritative` off — the first such prefab; a client copy's controller is disabled so the transform is written for it), `pose` (a byte, `CreaturePose`), `targetId` (the diver it is after), `strikeSerial` (a hit landed); the Lure and the Listener carry `CreatureBolts` with a `BeamCue { Serial, From, To, StartTick, Dark, Phase }` SyncVar (the one-shot serial idiom, a new serial per phase: `Charging` → `Firing` → `Done`), a `beamAim` `SyncVar<Vector3>` (the live aim point, server-written every frame while the beam charges and burns) and a `hitSerial` — **22 September 2026 (Dan):** the charge lasts `beamChargeSeconds`, the beam `beamSeconds`, and the server turns its aim after the target diver (`ServerAim`'s owner id: the lit diver, the noise's source, or the nearest diver) at `beamSweepDegPerSec` throughout; while it burns the server judges every frame and hurts **once per beam**: the first diver within `beamHitRadius` of the line before the first wall, not on safe ground, with a clear line from the beam to the chest (`MonsterBeamView` draws the charge glow, the thread, the beam easing after `beamAim` and the flare on `hitSerial`; `CreatureSounds` the charge and the loop — presentation, reading the SyncVars). "Watched" for the Angel is any of its body's bounds' corners inside a living diver's camera frustum (`watchVerticalFovDeg`, the widest assumed `watchAspect`, `watchPadDeg`) with a clear line. The Impostor adds `wornColour` (a palette index) and `wornName`; **who sees it is decided per camera on every peer** (`ImpostorLook`, like `SpectatorView`'s head): the chosen diver's own camera, a dead spectator's watching that diver, the deck TV's while its channel is that diver — the object itself replicates to everyone (no observer condition; the repo's observer rebuilds have bitten before). Deaths go through `WorldSceneFlow.ServerKill` (the killers) and `ServerDamage` (the rest); a creature never comes within `safeZoneMeters` of the shaft and never strikes a diver in the car or on the tube's floor. The look (`CreatureLook`: eyes by pose, bob, shake) and the sounds (`CreatureSounds`) read the SyncVars and decide nothing. The editor's checks force the draw with `MonsterSettings.RosterOverrideForTests` (every other matrix runs with an empty draw) |
| The monster grab (Long Walker, Weeping Angel) | **Server only** | **PROPOSED 24 September 2026, awaiting a teammate's review (built on `dan/monster-look`).** A creature whose prefab carries a `CreatureGrab` holds the diver before the kill. `Creature.Strike` catches only a diver `WorldSceneFlow.ServerCanKill` would take, never one already held, and writes `HQPlayerController.grabHold`, a server-written `SyncVar<GrabHold { Serial, Active, HolderId, StartTick, CaughtAt }>`. While it holds, the brain does not run and `pose` is `Grabbing`. At `holdSeconds` it calls `ServerKillHeld`: the kill ignores where the capsule went, keeps only refusals no position decides (no server or day, the player gone or already dead, off the site), and lays the body at the hold point. `ServerKill` shares the same kill body and behaves as before. A refused kill, a victim who died or left, or a despawn lets go (`ServerReleaseGrab`). The owner obeys the replicated hold: input, dash, jump and targets lock, the CharacterController goes off, the root goes to the hold point, and any dash in progress is cut. The server judges no dash while held (`ServerJudgeDash` is skipped). The rider lists of both cabin rides skip a held diver, and `ServerUnstuck` refuses "Held". The held eyes are computed on every peer from the holder's transform, `CreatureGrab.HoldPose(CaughtAt, seconds since StartTick)` and a shake seeded by the serial; `EyePose` returns them, so the diver, a spectator's `SpectatorView` and the `ShipTV` show one hold. No client writes any of it. Test seams: `Creature.RefuseGrabKillForChecks` and `HQPlayerController.GrabApplyDelayForChecks` |
| The Charger's knock-back | **Server only** (the shove is simulated by the owner) | **PROPOSED 24 September 2026, awaiting a teammate's review.** On a hit the server (`Charger` through `Creature.Strike`) calls `HQPlayerController.ServerKnockback(push)`. It writes `knockbackCue`, a server-written `SyncVar<KnockbackCue { Serial, Push, Tick }>`, and sends the owner a TargetRpc. The owner, which simulates its own movement, feeds the push through the motor's external-motion sweep (walls stop it) and damps its own steering meanwhile; the NetworkTransform carries the shove to everyone else. The view jolt is presentation, computed on every peer from the cue's server tick, and `EyePose` adds it, so spectators and the TV show it. A dead or held diver is never knocked back |
| The beams (Listener dark, Lure light), 24 September 2026 changes | **Server only** | **PROPOSED 24 September 2026, awaiting a teammate's review.** They amend the beam lines in "Monster AI and targeting". **Origin:** the judged line starts where the beam is drawn, `CreatureBolts.Origin` through `CreatureRig.BeamOriginPoint`, judged in `LateUpdate` after the rig.; **Hit:** the drawn beam touching the diver's capsule, the axis within `halfWidth` plus the capsule radius. `beamHitRadius` is now only a fallback for a diver with no capsule.; **Aim updates:** `beamAim` is sent at 60 Hz.; **Tracking:** the server tracks the target diver at `beamSweepDegPerSec`.; **The dash break:** a dash that starts during the charge or the burn, judged from the server copy's speed (`dashBreakMetresPerSecond`) or `ServerDashing` but never a knock-back, breaks the lock for that beam. The beam holds its heading through the dash, then trails at `reacquireMetresPerSecond`.; **Looks, presentation only:** the light beam's lighting (`MonsterBeamLight`, pooled lights) and the dark beam's darkening (`MonsterBeamShade`: a haze, a wall mark, and nearby Light components dimmed per camera render and restored exactly) read the replicated beam state on every peer. They never write `lampOn` or any SyncVar, and the Lure's senses read only `lampOn` |
| Elevator Ghost | **Server only** | **20 September 2026.** `GhostPhase { Serial, Active, StartTick, DurationTicks, SlamSerial }` on `CrewDayState`, written only by `WorldSceneFlow.Ghost`: when the car reaches the bottom on an **empty return trip** (`riding` false — never the ride down) the server rolls `ghostChance` at most once a day and, on a hit, opens a window of `ghostSeconds`; every server frame of it a living diver below whose object stands inside the car dies (`ServerKill`, "taken by the Elevator Ghost") and the window ends with `SlamSerial` moved; it ends by itself when its time is up or the car leaves the bottom. Clients derive the green interior light, the hum and the slam from the phase (`ElevatorGhostLight`, presentation, tick-anchored like the car). The checks force the roll (`GhostChanceOverrideForTests`) or summon it |
| Headlamp switch | **Server only** | **20 September 2026.** `HQPlayerController.lampOn` is a server-written `SyncVar<bool>` (on again at every `ServerBeginDive`) fed by the owner's `[ServerRpc] ServerRequestLamp(bool)` on F; every peer applies it to its copy's lamp (a remote copy's lamp shows while its diver is listed below and alive — its Unity scene on a client is not its world), and the server reads it for the Lure and the sight ranges. The lamp's range and intensity stay the upgrade's (`PlayerUpgrades`) |
| The dash | **Client-simulated (movement); server-judged** | **21 September 2026 (docs/DESIGN.md §3).** The owner's Alt moves its own capsule — `HQPlayerController.TryDash` sets a burst the motor applies for `dashSeconds` (`dashMeters` flat, and a kick of `dashLiftSpeed` on the vertical speed that gravity takes back: the little lift; a client simulation exception like the rest of movement; the `NetworkTransform` carries it; no flag goes to the server). **The server judges a dash from the copy's own speed**, as it judges sprinting: the flat ground the copy covered over the last 0.3 s reaching 60 % of a dash's length (`ServerJudgeDash`, every copy, the host's own included), once per burst — then it spends the air (`PlayerVitals.ServerSpendAir(dashAirSeconds)`, the leak multiplier on it), emits the `Dash` noise (`dashRadius`, the item's `ObjectId` as source; `PlayerNoise` emits no footsteps while `ServerDashing`) and writes `dashCue` (`SyncVar<DashCue { Serial, Direction }>`, server-written, the one-shot serial idiom). Every other peer plays the rings and the whoosh from the cue (`PlayerDashEffects`, presentation, world objects — a friend, a spectator and the TV see them); the owner plays its own at the press and ignores its own cue. The visor's DASH bar reads `DashReady` from the cue on every peer. The cooldown, the crouch, the two-handed grip and "only below" are the owner's refusals; a client without them only pays air for every burst the server sees. A move over 3 m in a frame is a teleport, never a dash |
| A landing's noise | **Server only** | **21 September 2026.** The thrower simulates a Released item (section 2), so the server's collisions never see it land: `CarryableItem.ServerJudgeLanding` (server, `FixedUpdate`) judges the landing from the copy the `NetworkTransform` brings — falling at `landingSpeed` or more, then not falling for three fixed steps — once per `motionVersion`, in the dive world, and emits an `Impact` at `landingRadius` scaled by the fall (never under half) with the item's `ObjectId` as source. Nothing replicates; nothing is asked of the thrower. `NoiseEmitter`'s collision path stays for world objects the server itself simulates |
...
| The court | **Server only** | **26 September 2026 correction, teammate review pending.** `CrewDayState.Baskets` remains server-written and resets with `ServerResetRun`. `HoopScore` observes successive server-visible ball positions: a `Free` or `Released` basketball must cross the rim downward within its clearance, then the lower plane; one shared basket per pass with repeat protection. It reads the existing owner's replicated transform for a client-thrown ball, never its server-side kinematic velocity, and never simulates that ball. Held/stowed/transit items, other scenes, upward passes, large jumps and motion-version changes are excluded. No new RPC or SyncVar. Clients/late joiners display the existing replicated count on both backboards. Other platform presentation remains local on each peer |
...
| Presses in general | **Server only** | Every `ShipControls` `[ServerRpc]` (sail, End day, pay, cabin, car, TV) first refuses a dead presser ("The dead press nothing"; 18 September 2026 — E itself is off while dead, so only a hand-made request reaches it). Range is the coarse test each press already has — aboard, inside the deck cabin, inside the car — not a per-button reach: a modified client aboard could press the monitor from the bow. Accepted for the prototype (invite-only crews); a per-button reach is the change if it ever matters. **Proposed 23 September 2026, awaiting teammate review** (docs/CONVENTIONS.md: a second pair of eyes on every contract change; built on `dan/ship-look`; played in the spectate matrix, rows T1r/T1/T2, on 24 September 2026; not approved): except the TV screen: `ShipControls.ServerRequestTvNext` also checks the presser's eyes within `TvReach` (6 m) + 1.5 m of the screen (`HQPlayerController.ServerCanReachTv`, called there) |
| Seats (the couch) | **Server only** | **Proposed 23 September 2026, awaiting teammate review** (docs/CONVENTIONS.md: a second pair of eyes on every contract change; built on `dan/ship-look`, not yet play-tested; not approved). `HQPlayerController.seat` (`SyncVar<byte>`, 0 = standing) is written only by the server: `[ServerRpc] ServerRequestSit(number)` (alive, not travelling, not a rider, not below, the seat in the ship of the object's scene, within InteractReach + 2 m of the copy's eyes, not taken in that scene; a refusal is `TargetSeatRefused`), `ServerRequestStand`, and a four-times-a-second check that stands anyone dead, riding or below, or more than 1.5 m from the seat (1.5 s grace; kept through a sail). The owner sits only after the server agrees and moves itself (client movement, the capsule off like a rider, the NetworkTransform carries it). Peers pose the copy from the SyncVar. The zoom (camera FOV), including a spectator's of a seated target, is local presentation. The use highlight is owner-local presentation |
...
- Server startup/shutdown must set/reset the bus's server guard and clean up
  ...
  moves (radius `elevatorRadius`, 60 m); **added 21 September 2026:** a
  `Dash` from `HQPlayerController.ServerJudgeDash` once per burst the server
  sees in the copy's speed (radius `dashRadius`, 20 m), an `Impact` from
  `CarryableItem.ServerJudgeLanding` once per release when the copy stops
  falling (radius `landingRadius`, 10 m, scaled by the fall). Radii live in
  `NoiseSettings` (Resources). `NoiseEmitter`'s collision path is guarded by
  `IsServer`.
...
- Hearing uses the bus. This does not forbid navigation, collision queries, or
  ...
  **Approved in the design (20 September 2026, §6 "The monsters"):** sight (a
  lamp-lit diver to `sightMeters`, a dark one to `sightDarkMeters`, a clear
  line on the world's colliders), being watched (the Angel: inside a living
  diver's `EyePose` cone with a clear line), light (the Lure: a lit lamp) and
  hearing (the Listener, the first `INoiseListener` in the game: it hears an
  event whose radius reaches it — a walking step at 6 m, a sprinting one at
  15 m, a landing at 10 m, a dash at 20 m, the car's scream at 60 m — and
  nothing from a crouch). All server-side in `CreatureSenses`; nothing about
  a sense replicates.

## 9. Elevator, oxygen and spectating

- **The Elevator Ghost (20 September 2026):** an empty return trip may arrive
  green (section 3, "Elevator Ghost"); the car's phase is untouched — the
  doors open as always — and a living diver who steps in during the green dies
  through the one death path. Never the ride down, never a first arrival.
...
- Order on the server: refuse unless every active connection's player stands on
  the deck proper (`SafeDeckVolume`, 9 m tall so the bridge tower's roof
  counts — the ship's stair came off on 23 September 2026; this wording is a
  factual correction, **proposed, awaiting teammate review**; the HQ's bridge
  and its landing are the base, not the ship —
...
- **25 September 2026 HQ art branch - derived boarding barriers (teammate
  ...
  **26 September follow-up (teammate review pending):** the HQ-owned final
  3.2 m bridge leaf uses `DockGangway`, evaluated from the same departure state
  and synchronized stage progress as the ship. It raises during `RaisingGangway`,
  stays raised through pull-away/loading, and lowers during `Arriving` at HQ.
  Both gates remain closed until travel completes; cancellation restores the
  docked pose. A kinematic local hinge body carries its collision only, never
  players/cargo. Its whole footprint is part of the fixed bridge exclusion.
  There is still no new RPC, SyncVar or gameplay writer.
