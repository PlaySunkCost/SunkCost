...
- Death exists (17 September 2026): `ProximityVoice.WorldOf` puts the dead in a
  ...
  proximity set, mixed at the target's head) and `Dead` (a dead speaker to every
  dead listener anywhere, full gain, no place). A living client discards
  `Spectate`/`Dead` frames; a dead client discards `Direct` ones.
