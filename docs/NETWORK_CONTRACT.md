...
| Player display name | Server: seeds `Diver N` when the player spawns, then writes the owner's request once, sanitised (`PlayerIdentity.Sanitize`: trimmed, printable, no `<>`, at most 16 characters, empty falls back) | The owner's client sends `ServerRequestDisplayName(name)` on its player's `OnStartClient` from the name saved on that machine (PlayerPrefs; the Steam persona by default, edited in the lobby before entering a room). `SyncVar<string>` on `PlayerIdentity`; clients only display it (the visor's crew tags, the roster). Names need not be unique; the server never trusts the string as sent. |
